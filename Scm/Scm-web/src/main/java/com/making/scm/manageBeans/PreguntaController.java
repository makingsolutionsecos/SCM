...
public class PreguntaController implements Serializable {
    ...
    @EJB
    private com.making.scm.accesoDatos.PreguntaDal preguntaDal;
    ...
    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/properties").getString("PreguntaCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/properties").getString("PreguntaUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/properties").getString("PreguntaDeleted"));
        if (!JsfUtil.isValidationFailed()) {
        ...
        if (selected != null) {
            ...
            try {
                ...
                if (msg.length() > 0) {
                    ...
                    JsfUtil.addErrorMessage(ex, ResourceBundle.getBundle("/properties").getString("PersistenceErrorOccured"));
                }
            } catch (Exception ex) {
